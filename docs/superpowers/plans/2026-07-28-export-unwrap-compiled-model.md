...
ssh martin@gx10-singularity.skunk-mercat.ts.net "cd ~/repos/tinyml-tensorlab && git pull"
```

- [ ] **Step 2: Re-run the full modelmaker test suite on the GX10**

```bash
ssh martin@gx10-singularity.skunk-mercat.ts.net "cd ~/repos/tinyml-tensorlab/tinyml-modelmaker && ~/repos/tinyml-tensorlab/.venv/bin/python -m pytest tests/ -v --tb=short --ignore=tests/test_protocols.py 2>&1 | tail -20"
```
...
```bash
ssh martin@gx10-singularity.skunk-mercat.ts.net "cd ~/repos/tinyml-tensorlab/tinyml-modelmaker && ~/repos/tinyml-tensorlab/.venv/bin/python validate_compile_amp_quant.py > /tmp/validation_final.log 2>&1; echo EXIT:\$?"
ssh martin@gx10-singularity.skunk-mercat.ts.net "grep -A 15 '^SUMMARY$' /tmp/validation_final.log"
```
